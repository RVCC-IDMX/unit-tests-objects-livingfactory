...
function getThreeBandValue(bands) {
  const num1 = getColorValue(bands.color1);
  const num2 = getColorValue(bands.color2);
  let finalString = `${num1}${num2}`;

  finalString *= getMultiplierValue(bands.multiplier);
  return Math.round(finalString * 100) / 100;
}
...
function formatNumber(val) {
  let newVal = 0;
  if (val >= 1000000000) {
    newVal = `${(val / 1000000000).toFixed(1).replace(/\.0$/, '')}G`;
    return newVal.toString();
  }
  if (val >= 1000000) {
    newVal = `${(val / 1000000).toFixed(1).replace(/\.0$/, '')}M`;
    return newVal.toString();
  }
  if (val >= 1000) {
    newVal = `${(val / 1000).toFixed(1).replace(/\.0$/, '')}k`;
    return newVal.toString();
  }
  if (val < 1000) {
    return val.toString();
...
function getTolerance(color) {
  ...
  const toleranceVar = `±${toleranceCodes[color]}%`;
  return toleranceVar;
}
...
function getResistorOhms(bands) {
  const resistVal = formatNumber(getThreeBandValue(bands));
  const tolerantVal = getTolerance(bands.tolerance);

  const returnString = `Resistor value: ${resistVal} Ohms ${tolerantVal}`;
  return returnString;
}
